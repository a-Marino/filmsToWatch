<template>
    <div class="nav flex items-center md:p-10 pt-2 px-2 md:h-12 h-auto min-w-full justify-between">
        ...
        <div class="relative overflow-hidden">
            <input type="text" class="search w-64" placeholder="Search" v-model="query" @keyup="getResults(query)">
        ...
        <div class="absolute right-10 top-16 w-64 resultados text-sm" v-if="results.length > 0">
            <ul v-for="movie in results.slice(0,3)" :key="movie.id">
                <li class="border-b border-gray-700" v-if="movie">
                    <router-link :to="'movie/'.movie.id" class="block px-3 py-3 hover:bg-gray-900 rounded">{{movie.title}}</router-link>
                </li>
            </ul>
        </div>
    </div>
