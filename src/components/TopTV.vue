<template>
    <div class="px-10 pt-5 flex flex-col top justify-center" v-if="popularTV">
        ...
        <div class="flex lg:flex-row flex-col lg:space-y-0 space-y-7 items-center">
            <div v-for="(show, Index) in popularTV.slice(0, 6)" :key="Index">
                <router-link :to="'/tv/'+show.id">
                    <div class="relative px-3">
                        <div class="rating space-x-3">
                            <h2 class="text-sm">{{show.vote_average}}</h2>
                        ...
                        <img :src="'https://image.tmdb.org/t/p/w500/'+show.poster_path" alt="poster" class="poster">
                    </div>
...
export default {
    ...
    mounted() {
        this.axios
        .get(`https://api.themoviedb.org/3/tv/popular?api_key=${process.env.VUE_APP_APIKEY}&language=en-US&page=1`)
        .then(res => (
